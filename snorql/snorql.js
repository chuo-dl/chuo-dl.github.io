...
function Snorql(){
	...
	this.init = function(pfxtext){
		...
		if(Util.init) Util.init(this.homedef, this.elts, uqparam);	//uqparam 2022-12-19
	...
	this.displayBooleanResult = function(value, resultTitle){
		...
		//if(value)	//string "false" evaluated to TRUE 2023-07-12
...
function SPARQLResultFormatter(json, namespaces, app){
	...
	this.is_image_var = {};	//object to hold if a var name is an image (set in _createTableHeader) 2022-11-27
	this.has_image_at_last = false;	//if image var is the last of the var set (= can be a gallery) 2022-11-27
	...
	this._getLinkMaker = function(varName, qname){
		...
		} else if(this.is_image_var[varName] && Util.queries && !this.app.qp_endpoint){//varName.match(/^(thumb(nail)?|image)$/)
	...
	this._createTableHeader = function(){
		...
		for(var i = 0, n=this._variables.length; i < n ; i++){
			var th = document.createElement('th'),
			varName = this._variables[i],
			is_image_var = varName.match(/^(thumb(nail)?|image)$/);
			if((this.is_image_var[varName] = is_image_var) && i === n-1){
				this.has_image_at_last = true;
			}
			
			th.appendChild(document.createTextNode(varName));
			tr.appendChild(th);
			if(this._variables[i] === 'namedgraph'){
				hasNamedGraph = true;
			}
			else if(is_image_var && Util.cls_toggler){	//this._variables[i] === 'image'
				//toggle to expand thumbnail 2019-12-20
				th.addEventListener("click", function(){
					//Util.cls_toggler(this.parentNode.parentNode, "expandimg");
					Util.cls_toggler(this.closest("table"), "expandimg");	//2022-09-07
				});
	...
	this._createTableRow = function(binding, rowNumber){
		...
		*/
		...
		for(var i = 0; i < numvars; i++){
			...
			if(this.is_image_var[varName]) td.classList.add("thumb");	//2022-11-27
			if(!node){
				//2020-07-05
